...
class CardModel extends AbstractModel {
	...
	public function update(Card $card) : ?Card {
		...
		$old_card = $this->read($card->id());

		if(!is_null($old_card)) {
			if($old_card->type_id() == $card->type_id()) {
				// throw new Exception("BRANCH 1");
				if(CardType::USER == $card->type_id()) {
				...
				// throw new Exception("BRANCH 2");
				if(CardType::USER == $old_card->type_id()) {
					$sql = 'DELETE FROM users_x_cards WHERE card_id = :id';
					$query = $connection->prepare($sql);

					$query->bindValue(':id', $old_card->id());
					$query->execute();
				}

				if(CardType::TRAINING == $old_card->type_id()) {
				...
				if(CardType::USER == $card->type_id()) {
					...
					$query->bindValue(':card_id', $card->id());

					$query->execute();
				}
			}

			// Change the card's type_id in the card table
			$sql = 'UPDATE cards SET type_id = :type_id WHERE id = :id';
			$query = $connection->prepare($sql);
